...
$sql = 'SELECT id, titre FROM oeuvres';
$statement = $conn->query($sql);
...
<form method="post" action="ajout.php" enctype="multipart/form-data" onsubmit="return validateDates()">
    <div class="mb-3">
        <!-- Sélectionne automatiquement sans pouvoir le modifier le titre de l'oeuvre en fonction de l'id de l'oeuvre -->
        <label for="titre" class="form-label">Titre de l'oeuvre</label>
        <input  name="id_oeuvres" class="form-control" value="<?php $oeuvres['id']?>" disabled>
    </div>
    </div>
    <div class="mb-3">
